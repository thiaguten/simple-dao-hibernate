 ...
 */
public abstract class AbstractHibernateJpaPersistenceProvider
    extends AbstractHibernatePersistenceProvider
    implements HibernateJpaPersistenceProvider {
  ...
  private <ID extends Serializable, T extends Persistable<ID>> TypedQuery<T> queryRange(
      TypedQuery<T> query, int firstResult, int maxResults) {
    if (query != null) {
      if (maxResults >= 0) {
